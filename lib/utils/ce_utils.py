...
def generate_mask_cond(cfg, bs, device, gt_bbox):
    if cfg.MODEL.BACKBONE.CE_TEMPLATE_RANGE != 'CTR_POINT':
        raise NotImplementedError("参考CRT_POINT修改其他代码，以使维度对齐")
    ...
    if cfg.MODEL.BACKBONE.CE_TEMPLATE_RANGE == 'ALL':
        box_mask_z = None
    elif cfg.MODEL.BACKBONE.CE_TEMPLATE_RANGE == 'CTR_POINT': # 大概目前只有这个能用，为了节省
        ...
        box_mask_z = box_mask_z.repeat(1, 4) # for two-modal template and online template
