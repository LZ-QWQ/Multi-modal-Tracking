...
class MixFormer_RGBT(nn.Module):
    def __init__(self, backbone, box_head, fusion_vi, head_type="CORNER"):
        ...
        """
        super().__init__()
        self.backbone_v = backbone[0]
        self.backbone_i = backbone[1]
    ...
    def forward(self, template, online_template, search, run_score_head=False, gt_bboxes=None,  return_features = False):
        ...
        template_v, online_template_v, search_v = self.backbone_v(template[0], online_template[0], search[0])
        template_i, online_template_i, search_i = self.backbone_i(template[1], online_template[1], search[1])

        # 某一个很老版本的checkpoint可能需要
        # _, c, h, w = search_v.size()  # B, 768, 18, 18
        # search = self.fusion_vi(search_v, search_i)  # B, 2*H*W, C
        # search_v, search_i = torch.chunk(search, 2, 1)
        # search = search_v + search_i  # 先加吧
        # search = search.permute(0, 2, 1).view(-1, c, h, w).contiguous()
        search = self.fusion_vi(search_v.contiguous(), search_i.contiguous())

        # search shape: (b, 384, 20, 20)
        # Forward the corner head
        if return_features:
            return *self.forward_box_head(search), search_v, search_i, search
        else:
...
def build_mixformer_vit_rgbt(cfg, train=True) -> MixFormer_RGBT:
    backbone_v = get_mixformer_vit(cfg, train)  # backbone without positional encoding and attention mask
    backbone_i = get_mixformer_vit(cfg, train)  # backbone without positional encoding and attention mask

    # 用DCN会在开始的时候看起来不收敛, 反正最后也用attention, 不管了
    # fusion_vi = RGBT_Fusion_1(768 * 2, 768)  # 768对应 base_patch16
    # fusion_vi = DeformableAttentionFusion(768, num_feature_levels=2)
    # fusion_vi = Attention_Fusion_Bimodal(768, d_model=512, num_feature_levels=2)
    ...
    model = MixFormer_RGBT([backbone_v, backbone_i], box_head, fusion_vi, head_type=cfg.MODEL.HEAD_TYPE)

    if cfg.MODEL.RGBT_PRETRAINED_PATH != "" and train:
        ckpt_path = cfg.MODEL.RGBT_PRETRAINED_PATH

        ckpt = torch.load(ckpt_path, map_location="cuda")["net"]
        new_dict = {}
        for k, v in ckpt.items():
            if "pos_embed" not in k and "mask_token" not in k:  # use fixed pos embed
                if "backbone" in k:
                    k_v = k.replace("backbone", "backbone_v")
                    k_i = k.replace("backbone", "backbone_i")
                    new_dict[k_v] = v
                    new_dict[k_i] = v
                else:
                    new_dict[k] = v
