...
class Block_Shared(nn.Module):
    ...
    def forward(self, x_vi, t_h, t_w, s_h, s_w):
        """
        双模态在batch层cat
        """
        N = x_vi.size(0) // 2
        x_v, x_i = torch.split(x_vi, [N, N], dim=0)
        x_v = self.norm1_v(x_v)
        x_i = self.norm1_i(x_i)
        x = torch.cat([x_v, x_i], dim=0)
        x_vi = x_vi + self.drop_path1(self.attn(x, t_h, t_w, s_h, s_w))
...
def build_mixformer_vit_rgbt_shared(cfg, train=True) -> MixFormer_RGBT:
