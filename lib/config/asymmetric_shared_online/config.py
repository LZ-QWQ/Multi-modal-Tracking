...
# MODEL
cfg.MODEL = edict()
cfg.MODEL.TRACKER_PRETRAINED_PATH = ''
cfg.MODEL.SCORE_PRETRAINED_PATH = ''
...
cfg.MODEL.FUSION_LAYERS = 6
cfg.MODEL.FUSION_CLASS = "Attention_Fusion_Bimodal"
# TRAIN
cfg.TRAIN = edict()
cfg.TRAIN.TRAIN_SCORE = False
cfg.TRAIN.RGBT_TRACK = False
cfg.TRAIN.RGBT_TRACK_SHARED = True
cfg.TRAIN.AMP = False
...
# TEST
cfg.TEST = edict()
cfg.TEST.LOAD_FROME_TRAIN_RESULT = False # True则从指定的训练结果加载模型, 否则从./models加载
    ...
    if isinstance(base_cfg, dict) and isinstance(exp_cfg, edict):
        for k, v in exp_cfg.items():
            if k in base_cfg:
                ...
                base_cfg[k] = v
                print("[Warning] In config dict, \"{}\" not exist, create with {}".format(k, v))
