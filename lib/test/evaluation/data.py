import numpy as np
from lib.test.evaluation.environment import env_settings, EnvSettings
...
class RGBT_Sequence:
    ...
    def __init__(self, name, frames, dataset, ground_truth_rect, target_visible=None):
        self.name = name
        self.frames = frames
        self.dataset = dataset
        self.ground_truth_rect = ground_truth_rect
        ...
        self.target_visible = target_visible
        self.init_data = self._construct_init_data()
        self._ensure_start_frame()
    ...
    def _construct_init_data(self):
        init_data = {0: dict()}  # Assume start from frame 0
        init_data[0]["bbox"] = (list(self.ground_truth_rect[0][0]), list(self.ground_truth_rect[0][1]))
        return init_data
    ...
    def get_bbox(self, frame_num):
        # LZ using for get real bbox for every frames
        return {"init_bbox": (list(self.ground_truth_rect[frame_num][0]), list(self.ground_truth_rect[frame_num][1]))}
