...
from lib.config.mixformer_vit_rgbt_unibackbone.config import cfg, update_config_from_file
...
def parameters(training_yaml_name: str, tracking_yaml_name: str, model=None, search_area_scale=None):
    ...
    yaml_file = os.path.join(prj_dir, "experiments/mixformer_vit_rgbt_unibackbone/%s.yaml" % training_yaml_name)
