...
def calc_seq_err_robust_rgbt(pred_bb, anno_bb_vi, dataset, target_visible=None):
    pred_bb = pred_bb.clone()
    ...
    # if (pred_bb[:, 2:] == 0.0).any():
    #     for i in range(1, pred_bb.shape[0]):
    #         raise ValueError("这里可能处理一下, 大概这里的原工作操作是预测0面积的框的话直接用前一帧的, 为啥不直接在后处理做？")
    #         if (pred_bb[i, 2:] == 0.0).any() and not torch.isnan(anno_bb[i, :]).any():
    #             pred_bb[i, :] = pred_bb[i - 1, :]

    if dataset == "VTUAV":
        # 因为VTUAV是10间隔的稀疏标注 !!!
        pred_bb = pred_bb[0::10, ...]

    if pred_bb.shape[0] != anno_bb_vi.shape[0]:
        if dataset == "lasot":
            if pred_bb.shape[0] > anno_bb_vi.shape[0]:
                # For monkey-17, there is a mismatch for some trackers.
                pred_bb = pred_bb[: anno_bb_vi.shape[0], :]
            else:
                raise Exception("Mis-match in tracker prediction and GT lengths")
        elif pred_bb.shape[0] == anno_bb_vi.shape[0] - 1:
            # 为了解决从VOT toolkit结果出来 第一个 box直接给个1，删掉后导致不对的问题
            pred_bb = torch.cat([anno_bb_vi[0:1, 0, :], pred_bb], dim=0)
        else:
            print(
                "Warning: Mis-match in tracker prediction and GT lengths",
                pred_bb.shape[0],
                anno_bb_vi.shape[0],
                dataset,
            )
            # raise Exception("Mis-match in tracker prediction and GT lengths", pred_bb.shape[0], anno_bb_vi.shape[0], dataset)
            if pred_bb.shape[0] > anno_bb_vi.shape[0]:
                pred_bb = pred_bb[: anno_bb_vi.shape[0], :]
            else:
                pad = torch.zeros((anno_bb_vi.shape[0] - pred_bb.shape[0], 4)).type_as(
                    pred_bb
                )
                pred_bb = torch.cat((pred_bb, pad), dim=0)

    pred_bb[0, :] = anno_bb_vi[0, 0, :]  # using RGB

    err_center_vi = [None] * 2
    err_center_normalized_vi = [None] * 2
    err_overlap_vi = [None] * 2
    for idx, _ in enumerate(["RGB", "TIR"]):
        anno_bb = anno_bb_vi[:, idx, :]

        err_center_vi[idx] = calc_err_center(pred_bb, anno_bb)
        err_center_normalized_vi[idx] = calc_err_center(
            pred_bb, anno_bb, normalized=True
        )
        err_overlap_vi[idx] = calc_iou_overlap(pred_bb, anno_bb)

    err_center = np.minimum(err_center_vi[0], err_center_vi[1])
    err_center_normalized = np.minimum(
        err_center_normalized_vi[0], err_center_normalized_vi[1]
    )
    err_overlap = np.maximum(err_overlap_vi[0], err_overlap_vi[1])

    if target_visible is not None:
        target_visible = target_visible.bool()
        valid = (
            ((anno_bb_vi[:, 0, 2:] > 0.0).sum(1) == 2)
            & ((anno_bb_vi[:, 1, 2:] > 0.0).sum(1) == 2)
            & target_visible
        )
    else:
        valid = ((anno_bb_vi[:, 0, 2:] > 0.0).sum(1) == 2) & (
            (anno_bb_vi[:, 1, 2:] > 0.0).sum(1) == 2
        )
    ...
    for seq_id, seq in enumerate(tqdm(dataset)):
        ...
        for trk_id, trk in enumerate(trackers):
            ...
            results_path = os.path.join(
                trk.results_dir, seq.dataset, "{}.txt".format(seq.name)
            )
            ...
            # Calculate measures
            if len(anno_bb.shape) == 3:
                # print("[Info] using RGBT eval")
                (
                    err_overlap,
                    err_center,
                    err_center_normalized,
                    valid_frame,
                ) = calc_seq_err_robust_rgbt(
                    pred_bb, anno_bb, seq.dataset, target_visible
                )
            else:
                print("[Info] using RGB eval")
                (
                    err_overlap,
                    err_center,
                    err_center_normalized,
