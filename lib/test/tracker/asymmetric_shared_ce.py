...
class MixFormer(BaseTracker):
    def __init__(self, params, dataset_name):
        ...
        network = build_asymmetric_shared_ce(params.cfg, train=False)
    ...
    def track(self, image, info: dict = None):
        ...
        with torch.inference_mode():
            if self.params.vis_search == 1:
                out_dict, _, search_feat_v, search_feat_i = self.network(
                    self.template, self.online_template, search_list, return_features=True
                )
            else:
                out_dict, _ = self.network(
                    self.template, self.online_template, search_list, return_features=False
                )
        ...
        for idx, update_i in enumerate(self.update_intervals):
            if self.frame_id % update_i == 0:
                ...
                self.online_template = self.preprocessor.process(z_patch_arr_v, z_patch_arr_i)
        ...
        if self.save_all_boxes:
            ...
            if self.params.vis_search:
                return {"target_bbox": self.state, "search_feat": (search_feat_v, search_feat_i)}
            else:
