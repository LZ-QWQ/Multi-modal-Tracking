...
class RGBTTrainer(BaseTrainer):
    def __init__(self, actor, loaders, optimizer, settings, lr_scheduler=None, accum_iter=1, use_amp=False, shed_args=None):
        """
        args:
        ...
        if settings.local_rank in [-1, 0]:
            with open(self.settings.log_file, "a") as f:
                # 先这样把, json.dumps会报错, 因为里面的env还是一个类
                f.write(pprint.pformat(self.settings.__dict__, indent=4) + "\n")
    ...
    def cycle_dataset(self, loader):
        ...
        for data_iter_step, data in enumerate(loader, 1):
            ...
            if loader.training:
                ...
                if not self.use_amp:
                    ...
                    self.loss_scaler(
                        loss,
                        self.optimizer,
                        parameters=self.actor.net.parameters(),
            ...
            # update statistics
            # batch_size = data['template_images_v'][0].shape[0]
    ...
    def train_epoch(self):
        ...
        self.cycle_dataset(train_loader)

        val_loader = self.loaders[1]
        if self.epoch % self.settings.val_interval == 0:
            self.cycle_dataset(val_loader)
        ...
        if i % self.settings.print_interval == 0 or i == loader.__len__():
            ...
            for name, val in self.stats[loader.name].items():
                if self.settings.print_stats is None or name in self.settings.print_stats:
                    if hasattr(val, "avg"):
                        print_str += "%s: %.5f (%.5f)  ,  " % (name, val.val, val.avg)
