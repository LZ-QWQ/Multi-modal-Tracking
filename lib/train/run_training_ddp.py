...
def dist_init():
    env_dict = {key: os.environ[key] for key in ("MASTER_ADDR", "MASTER_PORT", "RANK", "WORLD_SIZE", "LOCAL_RANK", "LOCAL_WORLD_SIZE")}
    ...
    device_ids = list(range(int(os.environ["LOCAL_RANK"]) * n, (int(os.environ["LOCAL_RANK"]) + 1) * n))
