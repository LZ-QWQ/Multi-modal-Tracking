...
class LasHeR(BaseVideoDataset):
    """
    哎 这个数据集依旧阴间
    image rgb is none, skip, file name: train_data/TrainingSet/trainingset/blueboy85/visible/v000018.jpg
    image rgb is none, skip, file name: train_data/TrainingSet/trainingset/blueboy85/visible/v000019.jpg
    image rgb is none, skip, file name: train_data/TrainingSet/trainingset/blueboy85/visible/v000020.jpg
    在这个框架下不太好处理这几帧, 我选择抛弃 blueboy85
    """
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader):
        ...
        super().__init__("LasHeR", self.root, image_loader)

        # Keep a list of all classes
        self.sequence_list = sorted(glob.glob(os.path.join(self.root, "TrainingSet", "trainingset","*")))
        self.sequence_list.remove(os.path.join(self.root, "TrainingSet", "trainingset","blueboy85"))
