...
class M3FDseq(BaseVideoDataset):
    """
    The M3FD dataset. M3FD is an image dataset for detection. Thus, we treat each image as a sequence of length 1.
    """

    def __init__(self, root=None, image_loader=jpeg4py_loader):
        """ """
        root = env_settings().m3fd_dir if root is None else root
        super().__init__("M3FD", root, image_loader)

        image_path_list_v = sorted(glob.glob(os.path.join(self.root, "Vis", "*")))
        image_path_list_i = sorted(glob.glob(os.path.join(self.root, "Ir", "*")))
        image_path_list_vi = list(zip(image_path_list_v, image_path_list_i))
        anno_list = sorted(glob.glob(os.path.join(self.root, "Annotation", "*")))
    ...
    def get_frames(self, seq_id=None, frame_ids=None, anno=None):
        # M3FD is an image dataset. Thus we replicate the image denoted by seq_id len(frame_ids) times, and return a
...
if __name__ == "__main__":
    ...
    test = M3FDseq(image_loader=cv2.imread)
